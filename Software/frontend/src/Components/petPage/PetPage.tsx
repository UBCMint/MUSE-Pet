...
const PetPage: React.FC<PetModel> = () => {
  ...
  const [recordingInProgress, setRecordingInProgress] =
    useState<boolean>(false);
  const [recordingButtonText, setRecordingButtonText] =
  ...
  const [brainData, setBrainData] = useState<BrainData>(
    location.state as BrainData
  );
  const [isDead, setIsDead] = useState<boolean>(pet.isDead);
  const [happinessLevel, setHappinessLevel] = useState<number>(
    location.state.happinessLevel
  );
  const [tirednessLevel, setTirednessLevel] = useState<number>(
    location.state.tirednessLevel
  );
  ...
  const getBrainData = async (intervalId: number) => {
    const response = await axios.get<BrainData>(
      "http://localhost:9000/brainData"
    );
